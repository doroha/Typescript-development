...
(define (contains? l i)
  (if (empty? l) #f
      (or (eq? (car(car l)) i) (contains? (cdr l) i))))
...
(define find-first
  (lambda (lz-lst p)
    ...
    (if (p (head lz-lst))(head lz-lst)(find-first (tail lz-lst) p))
)
  )
  )
...
(define sqrt2
  (lambda (x init epsilon)
    (car(find-first (sqrt-lzl x init) (lambda (e)(good-enough? (car e) e epsilon))
    ))
  )
)
...
(define get-value
  (lambda (assoc-list key)
   (if (empty? assoc-list) 'fail
       (if (eq? (car(car assoc-list)) key) (cdr(car assoc-list)) (get-value (cdr assoc-list) key)))
  )
)
...
(define get-value$
  (lambda (assoc-list key success fail)
      (if (empty? assoc-list) (fail)
       (if (eq? (car(car assoc-list)) key) (success (cdr(car assoc-list))) (get-value$ (cdr assoc-list) key success fail)))
  )
)
...
(define collect-all-values-1
 (lambda (lists key)
   (if (empty? lists) '()
       (if (contains? (car lists) key) (cons (get-value (car lists) key) (collect-all-values-1 (cdr lists) key)) (collect-all-values-1 (cdr lists) key))))
  )
 ...
 (lambda (lists key)
  (if (empty? lists)'()
   (get-value$ (car lists)key
               (lambda (l) (cons l (collect-all-values-2 (cdr lists) key)))
               (lambda ()
