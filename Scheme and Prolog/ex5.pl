...
pick_me_up(Child_name,Phone):- 
        parents(Child_name, P1, _),
        parent_details(P1, Phone, true).

pick_me_up(Child_name,Phone):-
        parents(Child_name, _, P2),
        parent_details(P2, Phone, true).


% Signature: active_child(Name)/1
% Purpose: true when a child participates in at least two activities

active_child(Child_name) :- participate(Child_name, Activity1),
                            participate(Child_name, Activity2),
                            Activity1 \= Activity2. 
...
activity_participants_list(Activity_name, List):-
                 bagof(Child_name, participate(Child_name, Activity_name), List).
...
can_register(Child_name, Activity):-
        findall(Day, (participate(Child_name, Act), activity(Act,Day)), Days),
                activity(Activity, ADay),
                        not_member(ADay, Days).
