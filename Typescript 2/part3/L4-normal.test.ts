...
*/
    ...
    it('New test 1', () => {
        expect(bind(parseL4(`
            (L4 (if #t 100 (/ 100 0)))`), evalNormalProgram)).to.deep.equal(makeOk(100));
    });

    it('New test 2', () => {
        expect(bind(parseL4(`(L4 (let ((x 99) (condition #t)) (if condition x (- x 1))))`), evalNormalProgram)).to.deep.equal(makeOk(99));
    });  
    ...
    it('New test 3', () => {
        expect(bind(parseL4(`
            (L4 
                ...
                (define y (lambda (b) (x y b)))
                (if (= 98 98) 98 (y 98)))`), evalNormalProgram)).to.deep.equal(makeOk(98));
    });
});  
