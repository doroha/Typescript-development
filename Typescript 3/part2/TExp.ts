...
;; [Empty -> void]
*/
import { chain, concat, map, uniq, is } from "ramda";
...
const parseCompoundTExp = (texps: Sexp[]): Result<CompoundTExp> => 
    texps.indexOf('->') == -1 ? parseCompoundTuplesTExp(texps) : 
    parseProcTExp(texps) 

const parseCompoundTuplesTExp = (texps: Sexp[]): Result<TupleTExp> =>
    bind(parseTupleTExp(texps), makeResultForTupleTExp);  

const makeResultForTupleTExp = (tuples: TExp[]) : Result<TupleTExp> =>
    isEmpty(tuples) ? makeOk(makeEmptyTupleTExp()) :
    allT(isNonTupleTExp , tuples) ? makeOk(makeNonEmptyTupleTExp(tuples)):
    makeFailure(`Unexpected tuples`);  
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const unparseTupleTExp = (tuple : TupleTExp) :  Result<string | string[]> =>
            isEmptyTupleTExp(tuple) ? unparseTuple([]) : unparseTuple(tuple.TEs) 
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isTupleTExp(x) ? unparseTupleTExp(x) : 
        makeFailure("Never");
