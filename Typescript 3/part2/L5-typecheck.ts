// L5-typecheck
// ========================================================
import { equals, map, zipWith, join, chain, identity } from 'ramda';
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         isPrimOp, isProcExp, isProgram, isStrExp, isVarRef, parseL5Exp, unparse,
         AppExp, BoolExp, DefineExp, Exp, IfExp, LetrecExp, LetExp, NumExp,
         Parsed, PrimOp, ProcExp, Program, StrExp, isValuesExp, isLetValuesExp, ValuesExp, LetValuesExp, CExp, VarDecl } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, TupleTExp, makeEmptyTupleTExp, isNonTupleTExp, makeNonEmptyTupleTExp } from "./TExp";
...
export const typeOfValuesExp = (values: ValuesExp, tenv: TEnv): Result<TExp> => {   

    const checkTypes = (types: TExp[]) : Result<TExp> =>
            isEmpty(types) ? makeOk(makeEmptyTupleTExp()): checkNonTupleTExp(types)

    const checkNonTupleTExp = (types: TExp[]) : Result<TExp> =>
    allT(isNonTupleTExp, types) ? makeOk(makeNonEmptyTupleTExp(types)): makeFailure("Unexpected Types") 

    const typesOfVals = mapResult((val: CExp) => typeofExp(val, tenv), values.vals);

     return bind(typesOfVals,checkTypes);  
}

export const typeOfLetValuesExp = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => 
{    
    const vars = flatten(map((vb) => vb.leftSide, exp.bindings))  

    const vals = map((vb) => vb.rightSide, exp.bindings); 

    const varTEs = map((v) => v.texp , vars); 
    
    return typeofExps(exp.body, 
                makeExtendTEnv(map((v) => v.var, vars), varTEs, tenv));
};
