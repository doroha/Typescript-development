...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isLetrecExp(x) || isSetExp(x) || isLetValuesExp(x) || isValuesExp(x);
export const expComponents = (e: Exp): CExp[] =>
    ...
    isSetExp(e) ? [e.val] :
    isDefineExp(e) ? [e.val] :
    isValuesExp(e) ?  e.vals :
    isLetValuesExp(e) ? [...e.body, ...map((vb) => vb.rightSide, e.bindings)] :
...
export const makeValuesExp = (vals: CExp[]): ValuesExp =>
({tag: "ValuesExp", vals: vals}); 
export const isValuesExp = (x: any): x is ValuesExp => x.tag === "ValuesExp"; 

export interface ValuesBindings {tag: "ValuesBindings"; leftSide: VarDecl[]; rightSide: CExp;}
export const makeValuesBindings = (left: VarDecl[] , right: CExp): ValuesBindings =>
({tag: "ValuesBindings", leftSide: left, rightSide: right}); 
export const isValuesBindings = (x: any): x is ValuesBindings => x.tag === "ValuesBindings";

export interface LetValuesExp {tag: "LetValuesExp"; bindings: ValuesBindings[]; body: CExp[]; }
export const makeLetValuesExp = (bindingsVals: ValuesBindings[] , body: CExp[]): LetValuesExp =>
    ({tag: "LetValuesExp", bindings: bindingsVals, body: body});
...
const parseValuesExp = (vals: Sexp[]): Result<ValuesExp> =>
    bind(mapResult(parseL5CExp, vals),
             (cexps: CExp[]) => makeOk(makeValuesExp(cexps))); 
...
const parseLetValuesExp = (bindings: Sexp, body: Sexp[]): Result<LetValuesExp> =>
    ...
    safe2((bdgs: ValuesBindings[], body: CExp[]) => makeOk(makeLetValuesExp(bdgs, body)))
...
const parseValuesBindings = (bindings: [Sexp[], Sexp][]): Result<ValuesBindings[]> =>
    safe2((vds: VarDecl[][], vals: CExp[]) => makeOk(zipWith(makeValuesBindings, vds, vals)))
...
const unparseValuesBindings = (bindings: ValuesBindings[]): Result<string> =>
    bind(mapResult(bdg => safe2((vd: string, val: string) => makeOk(`(${vd} ${val})`))(unparseVarDecls(bdg.leftSide), unparse(bdg.rightSide)), bindings),
         (bdgs: string[]) => makeOk(join(" ", bdgs)));
...
const unparseValuesExp = (vs: ValuesExp): Result<string> =>
    bind(unparseLExps(vs.vals), (val: string) => makeOk(`(values ${val})`));  
