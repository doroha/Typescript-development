...
export const makeForExp = (loopVar: VarDecl, startVal: NumExp, endVal:NumExp, body: CExp): ForExp =>
    ({tag: "ForExp", var: loopVar, start: startVal, end: endVal, body: body});
...
import { makeStrExp } from '../imp/L3-ast';
...
//<ForExp> -> (for <VarDecl> <NumExp> <NumExp> <CExp>)
const parseForExp =(variable:Sexp, values_body:Sexp[]) :Result<ForExp> =>
    !isArray(variable) && isIdentifier(variable) && values_body.length === 3 ?
          bind(mapResult(parseL21CExp, values_body),
          (cexps: CExp[]) => isNumExp(cexps[0]) && isNumExp(cexps[1]) ? cexps[0].val <= cexps[1].val ?  makeOk(makeForExp(makeVarDecl(variable),cexps[0],cexps[1], cexps[2])):  
          makeFailure("Invalid start or end values for ForExp") : makeFailure("start value should to be less then end value")) : makeFailure("Invalid params for ForExp");
